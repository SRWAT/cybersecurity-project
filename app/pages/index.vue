...
async function analyze() {
  ...
  // สร้าง FormData เพื่อห่อไฟล์ก่อนส่ง
    ...
    <section id="detect" class="bg-[#080b10] px-12 py-24">
      ...
      <div class="grid grid-cols-2 gap-6">
        ...
        <div>
          ...
          >
            <template v-if="file">
              <img v-if="fileType === 'image'" :src="preview" class="max-h-48 mb-4 rounded" />
              <p v-else-if="fileType === 'video'" class="text-5xl mb-4">🎬</p>
              <p v-else-if="fileType === 'audio'" class="text-5xl mb-4">🎵</p>
              <p class="text-white text-sm text-center">{{ file.name }}</p>
            </template>
            ...
            <template v-else>
              ...
              <div class="flex gap-2 mt-6 flex-wrap justify-center">
                ...
                <span class="text-xs border border-[#1e2d3d] text-[#5a7a94] px-2 py-1">MP3</span>
                <span class="text-xs border border-[#1e2d3d] text-[#5a7a94] px-2 py-1">WAV</span>
              </div>
            </template>
